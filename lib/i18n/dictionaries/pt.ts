import type { Dictionary } from "./en";

export const pt: Dictionary = {
  nav: {
    home: "Início",
    about: "Sobre",
    howItWorks: "Como funciona",
    guides: "Guias",
    download: "Baixar",
    faq: "Perguntas frequentes",
    support: "Suporte",
  },
  home: {
    hero: {
      title: "Registre com calma os lugares por onde seu dia passou",
      deviceText: "Momentos guardados com calma",
    },
    flow: {
      photo: {
        deviceText: "Capture um momento",
        text: "Salve uma foto ou uma nota curta quando quiser guardar aquele momento.",
      },
      moment: {
        deviceText: "O dia ganha forma",
        text: "Os momentos se agrupam por lugar e horário, e o dia vira uma história simples.",
      },
      memory: {
        deviceText: "Volte quando quiser",
        text: "Revise por data ou lugar. Sem metas, sem pressão.",
      },
    },
    pause: {
      text: "Sem sequências.\n\nDetecção de deslocamento e lembrete de fim da viagem quando você precisar.",
    },
    privacy: {
      text: "Privado por padrão. Só fica público quando você escolhe publicar.",
    },
    cta: {
      deviceText: "Comece um dia",
      primaryButton: "Baixar MomentBook",
      secondaryButton: "Baixar para Android",
    },
  },
  common: {
    comingSoon: "Em breve",
    learnMore: "Ver como funciona",
  },
  footer: {
    summary: "Lembre-se dos seus momentos.",
    ctaPrimary: "Baixar",
    social: {
      groupLabel: "Canais sociais do MomentBook",
      links: {
        youtube: "Ver MomentBook no YouTube",
        instagram: "Ver MomentBook no Instagram",
        tiktok: "Ver MomentBook no TikTok",
      },
    },
    sections: {
      product: "Produto",
      download: "Baixar o aplicativo",
      support: "Suporte",
      legal: "Legal",
    },
    email: "Email",
    links: {
      privacy: "Privacidade",
      terms: "Termos",
      communityGuidelines: "Diretrizes da comunidade",
      marketingConsent: "Consentimento de marketing",
      support: "Suporte",
    },
    copyright: "© 2026 MomentBook. Todos os direitos reservados.",
  },
};
